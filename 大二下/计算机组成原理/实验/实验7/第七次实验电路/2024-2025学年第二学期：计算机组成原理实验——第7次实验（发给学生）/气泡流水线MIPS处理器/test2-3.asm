#气泡流水线数据相关测试程序       test2-3.asm   （见教材的P267）
...
addi $v0,$zero,34                   #syscall的34号功能（在数码管上显示s1的值——1）
syscall

add $a0,$0,$s2		#要显示的值放在a0中
addi $v0,$zero,34                   #syscall的34号功能（在数码管上显示s2的值——1）
syscall

add $a0,$0,$s3		#要显示的值放在a0中
addi $v0,$zero,34                   #syscall的34号功能（在数码管上显示s3的值——2）
syscall

add $a0,$0,$s4		#要显示的值放在a0中
addi $v0,$zero,34                   #syscall的34号功能（在数码管上显示s4的值——5）
syscall

add $a0,$0,$s5		#要显示的值放在a0中
addi $v0,$zero,34                   #syscall的34号功能（在数码管上显示s5的值——1）
syscall               
