...
#MIPS程序    sort_sum_mips_bus.asm    排序（降序排列，从大到小）和求累加和程序                       
#先将5个数（4、1、5、2、3）存放到地址为1024开始的RAM存储器中（100存储单元），然后对这5个数进行排序，结果仍然放到原处
#然后求1+2+...+10=55=37H    结果保存到地址为1088RAM的存储器中（110存储单元）
